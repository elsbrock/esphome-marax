// Chart draw helpers: axis labels and shaded sections
#pragma once
...
static void chart_draw_event_cb(lv_event_t* e) {
    ...
    // Shade sections on the chart main layer after its background is drawn,
    // but before items/series (DRAW_PART_END for MAIN).
    if (code == LV_EVENT_DRAW_PART_END && dsc->part == LV_PART_MAIN) {
        if (!dsc->draw_ctx) return;
        uint32_t now_ms = millis();
        bool high = is_high_res_mode();
        uint32_t window_ms = (high ? HIGH_RES_WINDOW_SEC : LOW_RES_WINDOW_SEC) * 1000u;
        if (window_ms == 0) return;

        // Use content coords for a stable plot area instead of per-pass draw_area
        ...
        // Pass 1: draw brew windows (blue)
        for (int i = 0; i < n; ++i) {
            uint32_t t0 = ev[i].start_ms;
            uint32_t t1 = ev[i].stop_ms > 0 ? ev[i].stop_ms : now_ms;
            if (t1 <= vis_start || t0 >= now_ms) continue; // not visible
            ...
            if (x2 <= x1) continue; // nothing to draw
        ...
        // Pass 2: draw recovery windows (green). End on data-driven recovered_ms if present,
        // otherwise fall back to nominal recovery duration. Clamp to next brew start to avoid overlap.
        for (int i = 0; i < n; ++i) {
            ...
            // Clamp to next brew start if any
            if (i + 1 < n) {
            ...
            if (rx2c <= rx1c) continue; // nothing to draw
    ...
    if (code == LV_EVENT_DRAW_PART_BEGIN && dsc->part == LV_PART_TICKS && dsc->text) {
